 ...
 *
 */
public class FindSingleUniqueElementWhenAllOtherElementsAppearTwiceUsingSetCollection {
	...
	private static int findSingleUniqueElement(int[] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			// Check if element is there - if yes remove it from set as it's duplicate
			if (!data.add(arr[i])) {
				data.remove(arr[i]);
			}
		}
		return data.stream().findAny().orElse(-1);
	}
